...
async function loadGames() {
    try {
        const games = await apiClient.fetch('/api/games/');
        renderGames(games);
...
function renderGames(games) {
    ...
    for (const game of games) {
        ...
        row.innerHTML = `
            ...
            <div class="actions">
                <a class="button secondary" href="/client/games/form/?id=${game.id}">Editar</a>
...
async function deleteGame(id) {
    try {
        await apiClient.fetch(`/api/games/${id}/`, { method: 'DELETE' });
        await loadGames();
